...
const errorTranslations: Array<[string, string]> = [
  ...
  ['A disqualified participant cannot receive asset adjustments', '제재된 참가자의 자산은 변경할 수 없습니다.'],
  ['Participant asset adjustment request key was reused', '같은 자산 변경 요청 키가 다른 작업에 사용되었습니다. 다시 시도해 주세요.'],
  ['Participant asset adjustment amount must be positive', '지급·회수 수량은 0보다 커야 합니다.'],
  ['Participant asset adjustment reason must contain', '운영 사유는 5자 이상 500자 이하로 입력해 주세요.'],
  ['RP adjustment amount must be a safe positive integer', 'RP는 1 이상의 정수로 입력해 주세요.'],
  ['RP revocation exceeds the participant available cash', '예약 주문을 제외한 사용 가능 RP보다 많이 회수할 수 없습니다.'],
  ['Attendance token adjustment amount must be a positive integer', '출석토큰은 1개 이상의 정수로 입력해 주세요.'],
  ['Attendance token revocation exceeds the participant balance', '보유한 출석토큰보다 많이 회수할 수 없습니다.'],
  ['Stock adjustments require an open league round', '진행 중인 라운드가 열려 있을 때만 주식을 지급하거나 회수할 수 있습니다.'],
  ['An active listed stock in the participant league is required', '해당 리그에서 거래 중인 종목을 선택해 주세요.'],
  ['A participant cannot receive their own listed stock', '플레이어 본인이 상장한 종목은 지급할 수 없습니다.'],
  ['A participant with a short position cannot receive the same stock', '같은 종목의 공매도 포지션이 있어 주식을 지급할 수 없습니다.'],
  ['Stock revocation exceeds the unreserved non-leveraged position', '레버리지와 대기 매도를 제외한 회수 가능 주식보다 많이 회수할 수 없습니다.'],
  ...
  ['The selected participant has an existing position or open order in this stock', '이 종목의 보유·공매도 포지션 또는 미체결 주문이 있는 참가자는 상장자로 지정할 수 없습니다.'],
  ['Stock owner can only be assigned once after activation', '상장 후 지정한 상장자는 다시 변경하거나 해제할 수 없습니다.'],
  ...
  ['Stock identity, initial price, and logo are locked', '상장 확정 후에는 종목명·초기 가격·기본 이미지를 변경할 수 없습니다.'],
  ['Only a pending, active, or halted stock logo can be replaced', '설정 중·거래 중·거래 정지 종목의 로고만 교체할 수 있습니다.'],
  ['An uploaded stock logo cannot be removed', '상장 확정 후에는 업로드 로고를 삭제하거나 기본 이미지로 되돌릴 수 없습니다.'],
  ...
  ['AI market brief is missing or outside the allowed length', '글로벌 뉴스의 제목과 기사 길이를 확인해 주세요.'],
  ['Global news title, summary, and body must be entered together', '글로벌 뉴스 제목과 기사를 모두 입력해 주세요.'],
  ...
  ['Global news must be completed before settlement', '현재 라운드의 글로벌 뉴스 제목과 기사를 모두 입력해 주세요.'],
